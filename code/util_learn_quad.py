...
def quantize_points(points, dtype):
	if jnp.issubdtype(dtype, jnp.integer):
		# Integer quantization: Normalize to [-1, 1], scale, and dequantize
		int_min, int_max = jnp.iinfo(dtype).min, jnp.iinfo(dtype).max
		points_min, points_max = points.min(), points.max()
		points_scaled = (points - points_min) / (points_max - points_min) * 2 - 1
		points_int = jnp.round(points_scaled * int_max).astype(dtype)
		return points_int.astype(jnp.float32) / int_max * (points_max - points_min) + points_min

	elif jnp.issubdtype(dtype, jnp.floating):
		# Directly cast to lower precision floating point
...
def unpack_params(sizes, params):
	start = 0
	model_param = []
	for m,n in zip(sizes[:-1], sizes[1:]):
		in_dim = m
		out_dim = n
		bias_end = start + out_dim
		bias = params[start:bias_end].reshape(n)
		end = bias_end + out_dim*in_dim
		weight = params[bias_end:end].reshape((n,m))
		model_param.append([weight, bias])
		start = end
	return model_param
...
def dummyfn(x,y):
	value = 0.0000000000000000000001*x**2 + y
	return value
...
def plot_all(x_test_plot, y_test_plot, u_test_plot, u_pred_plot, epoch, exp_name):
...
def plot_along_weight(x_test_plot, y_test_plot, u_test_plot, u_pred_plot, alpha, beta, pre_quad_pt, mod_model, epoch, exp_name):
	...
	for a,b in zip(alpha, beta):
		wt_params = {'alpha':a, 'beta':b}
		weight_eval = vec_weight_fn(pre_quad_pt, mod_model, wt_params).squeeze(-1)
		axs[3].plot(weight_eval, label='a'+str(a)+'b'+str(b))
		axs[3].legend()
	plt.tight_layout()
	plt.savefig(''.join([exp_name,'/','combine_',str(epoch),'.png']))
	plt.close()
	
def plot_save_loss_dict(loss_dict, epoch, exp_name, filename='losse.png', plot_separate=True):
	loss_file = exp_name+'/all_losse.pkl'
	with open(loss_file, 'wb') as f:
		pickle.dump(loss_dict, f)

	if plot_separate:
		n_keys = len(loss_dict)
		n_rows = math.ceil(n_keys / 3)
		fig, axs = plt.subplots(n_rows+1, 3, figsize=(15, 4 * n_rows))

		# axs = axs.flatten() if n_rows == 1 else axs
		for i, (key, values) in enumerate(loss_dict.items()):
			row = i // 3
			col = i % 3
			axs[row, col].plot(values, color='red', label=key)
			axs[row, col].legend()

		# Remove empty subplots in the last row
		for i in range(n_keys, n_rows * 3):
			row = i // 3
			col = i % 3
			fig.delaxes(axs[row, col])

		# Create a subplot for all keys together in the last row
		for key in loss_dict.keys():
			axs[-1,1].plot(loss_dict[key], label=key)
		axs[-1,1].set_title("Combined Loss")
		axs[-1,1].legend()
		fig.delaxes(axs[-1, 2])
		fig.delaxes(axs[-1, 0])
		plt.tight_layout()
		plt.savefig(''.join([exp_name,'/',filename]))
		plt.close()
	else:
		for key in loss_dict.keys():
...
def compute_J_zero_beta_value(N_degree, alpha, beta, verbose=False):
...
# def root(k, mod_model, coeff_model, local_params, alpha, beta):
# 	vec_F = jax.vmap(F, in_axes=(None, None, 0, 0, None))
# 	bisec = Bisection(optimality_fun=vec_F, lower=-1, upper=+1, check_bracket=False)
# 	# vec_biec_run = jax.vmap(bisec.run, in_axes=(None, None, 0, 0, None))
# 	solve2 = bisec.run(k=k, mod_model=mod_model, a=alpha, b=beta, n=local_params['N'])
# 	solve = bisec.run(k=k, mod_model=mod_model, a=alpha, b=beta, n=local_params['N'])           # compute root
...
def root(k, mod_model, coeff_model, local_params):
	...
	post_solve_value = post_solve(solve.params, mod_model, coeff_model, local_params)
	return post_solve_value

def family_root(k, mod_model, coeff, local_params):
	bisec = Bisection(optimality_fun=F, lower=-1, upper=+1, check_bracket=False)
	solve = bisec.run(k=k, mod_model=mod_model, a=local_params['alpha'], b=local_params['beta'], n=local_params['N'])           # compute root
	post_solve_value = family_post_solve(solve.params, mod_model, coeff, local_params)
	return post_solve_value

def leading_order(k, mod_model, local_params, alpha, beta):
	bisec = Bisection(optimality_fun=F, lower=-1, upper=+1, check_bracket=False)
	solve = bisec.run(k=k, mod_model=mod_model, a=alpha, b=beta, n=local_params['N'])
	return solve.params
...
def F(tk, k, mod_model, a, b, n):
	...
	c2 = 4*n + 2*a + 2*b + 2
	c3 = 2*n + a + b + 1
	# F = (jnp.pi * c1) / c2 - jnp.arccos(tk) - (2*jnp.pi*jnp.log(modulation_net_forward_pass(tk, mod_model)+1e-4))/c3
	F = (jnp.pi * c1) / c2 - jnp.arccos(tk) - (jnp.log(modulation_net_forward_pass(tk, mod_model)+1e-8))/c3
...
def family_post_solve(root_tk, mod_model, all_coeff, local_params):
	Ntest_perelem = 1
	all_coeff = all_coeff.reshape(Ntest_perelem, -1)
	h_coeff = all_coeff[:, :2]
	c_coeff = all_coeff[:, 2]
...
# 	right_d = all_coeff[4:]
# 	pdb.set_trace()
# 	l_x_k = edge_node_xk(k, left_c,  left_d, local_params, True)
# 	l_w_of_xk = weight_fn(l_x_k, modulation_model, local_params)
# 	l_w_k = edge_weight_wk(k, left_c, left_d, l_w_of_xk, local_params, True)
# 	l_x_k = jax.lax.expand_dims(l_x_k, [-1])

# 	r_x_k = edge_node_xk(k, right_c,  right_d, local_params, False)
# 	r_w_of_xk = weight_fn(r_x_k, modulation_model, local_params)
# 	r_w_k = edge_weight_wk(k, right_c, right_d, r_w_of_xk, local_params, False)
...
def family_edge_compute(k, modulation_model, edge, local_params):
	all_coeff = edge
	Ntest_perelem = 1
	all_coeff = all_coeff.reshape(Ntest_perelem, -1)
	left_c = all_coeff[:, 0]
...
def tensor_prod_node(x_node, y_node):
	x_grid, y_grid = jnp.meshgrid(x_node, y_node)
	x_flat = jnp.ravel(x_grid)
	y_flat = jnp.ravel(y_grid)
	node_xk = jnp.stack((x_flat, y_flat), axis=1)
...
def cosine_similarity(x, y):
	...
	magnitude_x = jnp.linalg.norm(x)
	magnitude_y = jnp.linalg.norm(y)
	similarity = dot_product / (magnitude_x * magnitude_y)
	return similarity


def orthogonality_regularization(matrix):
	# Calculate the inner product of the matrix with its transpose
	inner_product = jnp.dot(matrix, matrix.T)
	# Calculate the Frobenius norm of the difference from the identity matrix
	frobenius_norm = jnp.linalg.norm(inner_product - jnp.eye(inner_product.shape[0]))
	# Return the Frobenius norm as the regularization term

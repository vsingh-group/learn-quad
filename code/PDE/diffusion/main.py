...
def parse_args():
	...
	parser.add_argument('--exp', default='Vdeg_noise_Diffusion', help="Adjusted in code: Experiment foler name")
	...
	parser.add_argument('--penalty', type=int, default=10, help='Weight for loss term due to boundary condition')
	parser.add_argument('--lr', default=0.0001, type=float, help="Learning rate for the asymptote model")
	...
	parser.add_argument('--N_degree', type=int, default=35, help='Degree of polynomial in one direction, which will give asymptotic node and weight')
	parser.add_argument('--min_degree', type=int, default=25, help='Minimum Degree of polynomial in one direction, used in varying case')
	...
	parser.add_argument('--solve_width', type=int, default=32, help='Per layer width of solution function which will be predicted')
...
def loss(t_model, fam_model, sol_model, train_data, mod_num, mod_width, mod_depth, sol_width, sol_depth, bc_penalty):
	...
	inhomogeneous_term = jax.vmap(inhomogene, in_axes=(0,0))(x_node.squeeze(), time.squeeze())
	domain_loss = jnp.mean((diffu*d2ux_value - dut_value + inhomogeneous_term)**2)
...
def main():
	...
	plot_res={"N_dergee":args.N_degree, "min_dergee":args.min_degree, "min_error":min_l2_error, "min_epoch":min_epoch}

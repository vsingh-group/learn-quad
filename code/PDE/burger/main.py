...
def parse_args():
	...
	'''
	parser = argparse.ArgumentParser(description="Arguments for Asymptote in PDE")
	parser.add_argument('--seed', type=int, default=52, help="Setting seed for the entire experiment")
	parser.add_argument('--exp', default='Burger_single_vdeg_direcsol_sample_time', help="Adjusted in code: Experiment foler name")
	...
	parser.add_argument('--lr', default=0.001, type=float, help="Learning rate for the asympttoe model")
...
def loss(t_model, fam_model, sol_model, train_data, mod_num, mod_width, mod_depth, sol_width, sol_depth, bc_penalty):
	...
	combine_node = tensor_prod_node(x_node, time)
	...
	dU_dx_value = dU_dx_vec(x_node.squeeze(), time.squeeze(), solver)
	...
	domain_loss = jnp.mean((diffu*d2ux_value - dut_value - Uvalue*dU_dx_value)**2)
...
def gen_testdata():
	...
	xx, tt = jnp.meshgrid(x.squeeze(), t.squeeze())
	X = jnp.vstack((jnp.ravel(xx), jnp.ravel(tt))).T
...
def main():
	...
	for epoch in range(args.NEpoch):
		prev_epoch_key, N_key = random.split(prev_epoch_key, 2)
		...
		# train_data = {"JL_zero": J_left_zeros, "JR_zero": J_right_zeros, "JL_beta": J_beta_value_left, "JR_beta": J_beta_value_right,
		# 			"K_bulk": K_bulk, "K_edge": K_edge, "degree": N_degree, "alpha": alpha, "beta": beta,
		# 			"pre_quad_pt": pre_quad_pt, "pre_quad_wt": pre_quad_wt, 'X': jnp.array([mu])}
		# update parameter

...
def parse_args():
	...
	parser.add_argument('--exp', default='time', help="Adjusted in code: Experiment foler name")
	...
	parser.add_argument('--penalty', type=int, default=10000, help='Weight for loss term due to boundary condition')
	parser.add_argument('--lr', default=0.001, type=float, help="Learning rate for the asymptote model")
	parser.add_argument('--min_max', default=False, action='store_true', help='Turn on simple min-max training')		
	...
	parser.add_argument('--N_degree', type=int, default=2000, help='Degree of polynomial in one direction, which will give asymptotic node and weight')
	parser.add_argument('--min_degree', type=int, default=1970, help='Minimum Degree of polynomial in one direction, used in varying case')
	...
	parser.add_argument("--dtype", type=str, choices=dtype_mapping.keys(), required=True, help="Choose a dtype for quantization.")
	...
	parser.add_argument('--solve_width', type=int, default=100, help='Per layer width of solution function which will be predicted')
	parser.add_argument('--solve_depth', type=int, default=5, help='Depth of solution function which will be predicted')
...
def init_distribution(x):
	value = jnp.sin(jnp.pi*x) + 0.5*jnp.sin(4*jnp.pi*x)
	return value
...
def loss(t_model, fam_model, sol_model, train_data, mod_num, mod_width, mod_depth, sol_width, sol_depth, bc_penalty, dtype):
	vec_family = jax.vmap(family_forward_pass, in_axes=(0, None))
	# X = jnp.expand_dims(train_data['X'], [0])
	X = jnp.expand_dims(train_data['noise'], [0])
	...
	x_node = (x_node+1)/2     # bring in 0 to 1

	# Quantize
	# x_node = quantize_points(x_node, dtype)
	...
	d2U_dt2=jax.grad(dU_dt, argnums=1, has_aux=False)
	d2U_dt2_vec=jax.vmap(d2U_dt2, in_axes=(0,0,None))
	d2ut_value=d2U_dt2_vec(x_node.squeeze(), time.squeeze(), solver) 

	dU_dt_vec = jax.vmap(dU_dt, in_axes=(0, None, None))
	dut_value = dU_dt_vec(x_node.squeeze(), 0.0, solver)

	diffu = train_data['X'][-1]
	domain_loss = jnp.mean((d2ut_value - diffu*d2ux_value)**2)
	gradient_loss = jnp.mean((dut_value - 0)**2)
	...
	left_temp_pred = jax.vmap(solution_net_forward_pass_v2, in_axes=(None, 0, None))(0.0, time_lin.squeeze(), solver)
	left_temp_true = jax.vmap(left_boundary, in_axes=(0, None))(time_lin.squeeze(), 0)
	...
	right_temp_true = jax.vmap(right_boundary, in_axes=(0, None))(time_lin.squeeze(), 0)
	right_bc_err = jnp.mean((right_temp_pred - right_temp_true)**2)

	ic_loss = left_bc_err + right_bc_err + gradient_loss + init_dist_lose
...
def main():
	...
	if args.min_max:
		solution_optim = optax.adam(learning_rate=-1.0*args.lr)
	else:
	...
	def joint_update(t_model, t_state, fam_model, fam_state, sol_model, sol_state, train_data):
		...
		((loss_value, (loss_dict, solve)), gradient) = loss_fn_grad_value(t_model, fam_model, sol_model, train_data, mod_num, mod_width, mod_depth, sol_width, sol_depth, bc_penalty, dtype)
	...
	for epoch in range(args.NEpoch):
		prev_epoch_key, data_key, noise_key, N_key = random.split(prev_epoch_key, 4)
		...
		train_data['noise']=jnp.array([noise])
	...
	plot_res={"N_dergee":args.N_degree, "min_dergee":args.min_degree, "min_error":min_l2_error, "min_epoch":min_epoch, "dtype":args.dtype}
	field_names = plot_res.keys()
	argument_storage_file = './Result/'+exp_name+'/degree_plot.csv'
	if os.path.exists(argument_storage_file):
		with open(argument_storage_file, 'a') as csv_file:
			dict_object = csv.DictWriter(csv_file, fieldnames=field_names) 
			dict_object.writerow(plot_res)
	else:
		with open(argument_storage_file, 'w') as csv_file:
			writer = csv.writer(csv_file) 
			writer.writerow(plot_res.keys()) 
			writer.writerow(plot_res.values())
	print(plot_res)

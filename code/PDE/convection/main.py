...
def parse_args():
	...
	parser.add_argument('--exp', default='conv_time', help="Adjusted in code: Experiment foler name")
	...
	parser.add_argument('--N_degree', type=int, default=1020, help='Degree of polynomial in one direction, which will give asymptotic node and weight')
	parser.add_argument('--min_degree', type=int, default=980, help='Minimum Degree of polynomial in one direction, used in varying case')
	...
	parser.add_argument('--family_width', type=int, default=50, help='Per layer width of modulation model')
	...
	parser.add_argument('--solve_width', type=int, default=50, help='Per layer width of solution function which will be predicted')
...
def loss(t_model, fam_model, sol_model, train_data, mod_num, mod_width, mod_depth, sol_width, sol_depth, bc_penalty, dtype):
	...
	x_node = ((x_node+1)/2)*(2*jnp.pi)     # bring in 0 to 1
	...
	dU_dx_vec = jax.vmap(dU_dx, in_axes=(0, 0, None))
	dux_value = dU_dx_vec(x_node.squeeze(), time.squeeze(), solver)
	...
	domain_loss = jnp.mean((dut_value + diffu*dux_value)**2)
	...
	right_temp_pred = jax.vmap(solution_net_forward_pass_v2, in_axes=(None, 0, None))(2.0*jnp.pi, time_lin.squeeze(), solver)
	Bc_loss = jnp.mean((right_temp_pred - left_temp_pred)**2)

	ic_loss = Bc_loss + init_dist_lose
...
def gen_testdata():
	X = jnp.load('convX.npy')
	y = jnp.load('convY.npy')
	# X = jnp.load('conv50X.npy')
	# y = jnp.load('conv50Y.npy')
	return X, y
...
def main():
	...
	for epoch in range(args.NEpoch):
		...
		epoch_time = sys_time.time()-start_time
